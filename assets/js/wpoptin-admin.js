...
jQuery(document).ready(function($) {
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if (response.success) {
          ...
          if ( ChangedStatus === 'publish' ) {
            $('.xo_wrap #wpop-preview').fadeIn();
          }else{
            $('.xo_wrap #wpop-preview').fadeOut();
          }
  ...
  var conversationHistory = []; // To keep track of the conversation

  $('#wpop_write_with_ai').on('click', function(e) {
      e.preventDefault(); // Prevent default behavior
      $('#wpop_ai_modal').modal('open');
  });

  $('#wpop_send_message').on('click', function(e) {
      e.preventDefault(); // Prevent default form submission behavior
      var userMessage = $('#wpop_user_input').val();
      if (userMessage.trim() !== '') {
          addMessageToChat('user', userMessage);
          $('#wpop_user_input').val(''); // Clear input field
          conversationHistory.push({ role: 'user', content: userMessage });
          generateAIContent(conversationHistory);
      }
  });

  $('#wpop_finalize_content').on('click', function(e) {
      e.preventDefault(); // Prevent default form submission behavior
      var finalContent = $('#wpop_chat_content').text();
      $('#wpop_new_content').val(finalContent);
      $('#wpop_ai_modal').modal('close'); // Close the modal after finalizing content
  });

  function addMessageToChat(role, content) {
      var messageClass = role === 'ai' ? 'ai' : 'user';
      $('#wpop_chat_content').append('<div class="message ' + messageClass + '">' + content + '</div>');
      $('#wpop_chat_content').scrollTop($('#wpop_chat_content')[0].scrollHeight); // Scroll to the bottom
  }

  function generateAIContent(history) {
      $.ajax({
          url: wpoptin.ajax_url,
          method: 'POST',
          data: {
              action: 'wpop_ai_generate_text_callback',
              conversation: history
          },
          success: function(response) {
              if (response.success) {
                  var aiMessage = response.data;

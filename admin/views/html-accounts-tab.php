                ...
                <div class="col s9">
                    ...
                    <ul>
								...
								if ( $terms ) {
									foreach ( $terms as $term ) { ?>
                                        <option value="<?php echo esc_attr( $term->slug )?>"
											<?php echo selected( $term->slug, $WPOptins->wpop_get_settings( $wpoptin_id, 'service_provider', false, false, $is_child ), false ) ?> ><?php echo esc_attr( $term->name )?>
                                        </option>
									<?php }
								} ?>
                            </select>
							...
							if ( isset( $wpoptin_id ) and isset( $goal ) ) {
								...
								*/
								...
								if ( $wpOptins->have_posts() ) { ?>
                                    ...
                                    <li class="xo_accounts_li xo_accounts_li_<?php echo esc_attr( $service_name )?>">
                                        ...
                                        <select class="xo_account_name">
											...
											<?php while ( $wpOptins->have_posts() ): $wpOptins->the_post(); ?>
                                                <option value="<?php echo esc_attr( get_the_ID() )?>"<?php echo selected( get_the_ID(), $WPOptins->wpop_get_settings( $wpoptin_id, 'account_id', false, false, $is_child ), false ) ?>><?php echo esc_attr( get_the_title() )?>
                                                </option>
											<?php endwhile; ?>
								...
								if ( isset( $data ) ) {
                                    ...
                                    <li class="xo_account_list_li xo_account_list_li_<?php echo esc_attr( $acount_id )?>">
                                        <label class="label"><?php esc_html_e( "Select mailing list", 'wpoptin' ); ?>
                                        </label>
                                        <select name="xo_account_list"
                                                class="xo_account_list">
                                            <option value="0"><?php echo esc_html( __( '--Select One--', 'wpoptin' ) )?>
                                            </option>
                        ...
                        <div class="xo_account_add_holder xo_account_add_holder_mailchimp">
                            ...
                            <a href="https://login.mailchimp.com/oauth2/authorize?response_type=code&client_id=544239805685&redirect_uri=https://wpxoptin.com/mc-vZEhNb7e8PFA/index.php&state=<?php echo esc_attr( $current_url )?>"
                        ...
                        <div class="xo_account_add_holder xo_account_add_holder_constant_contact">
                            ...
                            <a href="https://oauth2.constantcontact.com/oauth2/oauth/siteowner/authorize?response_type=code&client_id=<?php echo esc_attr( $cc_client_id )?>&redirect_uri=http://wpxoptin.com/optins-integrations-vZEhNb7e8PFA/cc-6WvtkhasGyJ5FNWj/index.php&state=<?php echo esc_url( $cc_current_url )?>"
                               class="btn waves-effect waves-light xo_acount_auth_page">
								<?php esc_html_e( "Authenticate", 'wpoptin' ); ?>
                                <i class="material-icons right">vpn_key
                                </i>
                        ...
                        <div class="xo_account_add_holder xo_account_add_holder_campaign_monitor">
                            ...
                            <a href="https://api.createsend.com/oauth?type=web_server&client_id=120333&redirect_uri=http://wpxoptin.com/optins-integrations-vZEhNb7e8PFA/cm-wYChvuR2wvj5/index.php&scope=ManageLists&state=<?php echo esc_url( $current_url )?>"
                               class="btn waves-effect waves-light xo_acount_auth_page">
								<?php esc_html_e( "Authenticate", 'wpoptin' ); ?>
                                <i class="material-icons right">vpn_key
                                </i>

...
<div class="xo_wrap z-depth-1">
    ...
    <div class="xo_header fadeIn">
        ...
        <div class="xo-addnew-right">
            ...
            <a
                href="<?php echo esc_url(admin_url('admin.php?page=wpop_overview'))?>"
                id="xo_stats"
                class="xo_stats wpop-tooltipped"
                data-position="bottom"
                data-tooltip="<?php esc_html_e("Analytics", 'wpoptin');?>">
            ...
            <a
                ...
                data-position="bottom"
                data-tooltip="<?php esc_html_e("Add New", 'wpoptin');?>">
...
$i++;
?>
                <tr class="row_<?php echo esc_attr( $wpoptins_account['ID'] )?> tr_parent">
                    <td>
                        <?php echo esc_attr( $wpoptins_account['title'] )?>
                    </td>
                    <td>
                        <?php echo esc_attr( $wpoptins_account['service_provider'] )?>
                    </td>
                    <td></td>
                    <td>
                        <div class="action_holder">
                            <a
                                href="javascript:void(0);"
                                id="xo_remove<?php echo esc_attr( $i )?>"
                                data-is_parent="true"
                                data-remove_id="<?php echo esc_attr( $wpoptins_account['ID'] )?>"
                                class="wpop_remove_db wpop-tooltipped"
                                data-position="right"
                                data-tooltip="<?php esc_html_e("Remove", 'wpoptin');?>">
        ...
        foreach ($wpoptins_account['lists'] as $single_list) { ?>
                <tr class="row_<?php echo esc_attr( $wpoptins_account['ID'] )?>">
                    <td>
                        <?php echo esc_attr( $single_list['name'] )?>
                    </td>
                    <td></td>
                    <td>
                        <?php echo esc_attr( $single_list['subscribers'] )?>
                    </td>
                    <td></td>
                </tr>
                <?php }
            ...
            <div class="xo_optin_accounts">
                <ul>
                    <li class="xo_account_provider_li">
                        ...
                        <select class="xo_account_provider_page">
                            <option value="0"><?php esc_html_e("--Select one--", 'wpoptin');?>
                            </option>
                            <?php $terms = get_terms( 'wpop_accounts_tax', [ 'hide_empty' => false] );
                if(isset($terms))
                {foreach ($terms as $term ) {  ?>
                            <option value="<?php echo esc_attr( $term->slug )?>"><?php echo esc_attr( $term->name )?>
                            </option>
                            <?php }} ?>
                        </select>
                    </li>
                    <div class="xo_lists_holder"></div>
                    <div class="xo_account_add_holder xo_account_add_holder_mailchimp">
                        <li class="xo_authBtn_li">
                            <a
                                href="https://login.mailchimp.com/oauth2/authorize?response_type=code&client_id=544239805685&redirect_uri=<?php echo urlencode('https://wpxoptin.com/mc-vZEhNb7e8PFA/index.php')?>&state=<?php echo esc_attr( $current_url )?>"
              ...
              $cc_current_url = urlencode_deep(admin_url('admin.php?page=wpop_new&goal=optin&type=tab&client_id='.esc_attr( $cc_client_id ).''));

              ?>

                        <a
                            href="https://oauth2.constantcontact.com/oauth2/oauth/siteowner/authorize?response_type=code&client_id=<?php echo esc_attr( $cc_client_id )?>&redirect_uri=http://wpxoptin.com/optins-integrations-vZEhNb7e8PFA/cc-6WvtkhasGyJ5FNWj/index.php&state=<?php echo esc_attr( $cc_current_url )?>"
                            class="btn waves-effect waves-light xo_acount_auth_page">
                            <?php esc_html_e("Authenticate", 'wpoptin');?>
                            <i class="material-icons right">vpn_key
                            </i>
                    ...
                    <div class="xo_account_add_holder xo_account_add_holder_campaign_monitor">
                        ...
                        <a
                            href="https://api.createsend.com/oauth?type=web_server&client_id=120333&redirect_uri=http://wpxoptin.com/optins-integrations-vZEhNb7e8PFA/cm-wYChvuR2wvj5/index.php&scope=ManageLists&state=<?php echo esc_attr( $current_url )?>"
                            class="btn waves-effect waves-light xo_acount_auth_page">
                            <?php esc_html_e("Authenticate", 'wpoptin');?>
                            <i class="material-icons right">vpn_key
                            </i>
                ...
                <span class="xo_sub_holder" style="display: none;">
                    <a
                        href="<?php esc_url(admin_url('admin.php?page=wpop_accounts')) ?>"
        ...
        <?php if( empty( $wpoptins_accounts ) ){ ?>
...
<div id="wpop-upgrade-mail-poet" class="modal wpop-upgrade-modal">
    <div class="modal-content">
        <div class="wpop-modal-content">
            ...
            <hr>
            <p><?php esc_html_e("Use following coupon code to get 27% discount for limited time", 'wpoptin');?><br>
                <code>BFCMOP</code>
            </p>
            <a
                href="<?php echo esc_url( wpop_fs()->get_upgrade_url() )?>"
                class="waves-effect waves-light btn z-depth-3">
                <i class="material-icons right">lock_open</i><?php esc_html_e("Upgrade to pro", 'wpoptin');?></a>
...
<h2><?php if( isset( $banner_info['name'] ) ){ echo esc_html( $banner_info['name'] ); } ?>
    <b><?php if( isset( $banner_info['bold'] ) ){ echo esc_html( $banner_info['bold'] ); }  ?></b>
</h2>
<p><?php if( isset( $banner_info['description'] ) ){ echo esc_html( $banner_info['description'] ); }  ?></p>
<p><?php if( isset( $banner_info['discount-text'] ) ){ echo esc_html( $banner_info['discount-text'] ); }  ?>
    <code><?php if( isset( $banner_info['coupon'] ) ){ echo esc_html( $banner_info['coupon'] ); }  ?></code>
</p>
<a
    href="<?php echo esc_url( wpop_fs()->get_upgrade_url() ) ?>"
    class="waves-effect waves-light btn">
    <i class="material-icons right">lock_open</i><?php if( isset( $banner_info['button-text'] ) ){  echo esc_html( $banner_info['button-text'] ); } ?>
</a>
</div>

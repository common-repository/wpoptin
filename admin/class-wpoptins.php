    ...
    class WPOptins {
        ...
        public function wpop_active_optins() {
            ...
            if ( $xOptins->have_posts() ) {
                ...
                while ( $xOptins->have_posts() ) {
                    ...
                    if ( 'woo_product' == $goal ) {
                        if ( isset( $custom_fields['_wpop_woo_product']['0'] ) ) {
                            $xo_holder[$id]['woo-product'] = $custom_fields['_wpop_woo_product']['0'];
                        } else {
                            $xo_holder[$id]['woo-product'] = null;
                        }
                    }
                    if ( 'optin' == $goal ) {
                    }
                    ...
                    if ( isset( $custom_fields['_wpop_timer']['0'] ) ) {
                        ...
                        if ( !is_numeric( $timer_enddate ) ) {
                            $timer_enddate = strtotime( $timer_enddate );
                        }
                        $xo_holder[$id]['timer-enddate'] = gmdate( 'Y/m/d', $timer_enddate );
                    } else {
                    ...
                    if ( isset( $skin_id ) ) {
                        ...
                        if ( is_array( $customizer_style ) ) {
                            $customizer_data['design'] = $customizer_style['design'];
                        } else {
                            // Handle the case when $customizer_style is not an array
                            $customizer_data['design'] = null;
        ...
        public function wpop_get_settings(
            ...
            if ( isset( $variant ) ) {
                ...
                if ( 'woo_product' == $goal ) {
                    if ( isset( $custom_fields['_wpop_woo_product']['0'] ) ) {
                        $xo_variants[$id]['woo-product'] = $custom_fields['_wpop_woo_product']['0'];
                    } else {
                        $xo_variants[$id]['woo-product'] = null;
                    }
                }
                if ( 'optin' == $goal ) {
                }
                ...
                /* If there is no data in live preview section of xOptin setting the default data. */
                if ( is_array( $xoptins_customizer ) ) {
                    $customizer_data['design'] = $xoptins_customizer['design'];
                    $customizer_data['border'] = $xoptins_customizer['border'];
                } else {
                    // Handle the case when $xoptins_customizer is not an array
                    $customizer_data['design'] = null;
                    // or assign a default value
                    $customizer_data['border'] = null;
                    // or assign a default value
                }
            ...
            if ( $group ) {
                if ( $variant ) {
                    ...
                    if ( isset( $wpoptins[$id] ) ) {
                        $setting_val = ( isset( $wpoptins[$id][$group] ) ? $wpoptins[$id][$group] : '' );
            ...
            if ( $group_child ) {
                if ( $variant ) {
                    ...
                    $setting_val = ( isset( $wpoptins[$id][$group][$group_child] ) ? $wpoptins[$id][$group][$group_child] : '' );
                }
            }
            if ( $key ) {
                if ( $variant ) {
                    ...
                    $setting_val = ( isset( $wpoptins[$id][$group][$group_child][$key] ) ? $wpoptins[$id][$group][$group_child][$key] : '' );
        ...
        public function optin_exists( $post_id, $post_type ) {
            ...
            // Check if the post exists and if the post type matches the provided post type
            if ( $post && get_post_type( $post_id ) === $post_type ) {
                return true;
            } else {
                return false;
            }
        ...
        /* wpoptins_conditions_defaults Method ends here. */
    }

...
if ( !class_exists( 'WPOptins_Admin' ) ) {
    class WPOptins_Admin {
        ...
        function __construct() {
            ...
            add_action( 'wp_ajax_wpop_save_woo_product', [$this, 'wpop_save_woo_product'] );
            ...
            add_action( 'wp_ajax_wpop_ai_generate_text_callback', [$this, 'wpop_ai_generate_text_callback'] );
            add_action( 'wp_ajax_nopriv_wpop_ai_generate_text_callback', [$this, 'wpop_ai_generate_text_callback_callback'] );
        }
        ...
        public function wpop_admin_style( $hook ) {
            ...
            if ( $hook == 'toplevel_page_wpoptin' or $hook == 'wpoptin_page_wpop_new' or $hook == 'wpoptin_page_wpop_accounts' or $hook == 'wpoptin_page_wpop_skins' ) {
                ...
                if ( isset( $_SERVER['HTTP_HOST'], $_SERVER['REQUEST_URI'] ) ) {
                    // Sanitize HTTP_HOST and REQUEST_URI, then construct the current link
                    $protocol = ( isset( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http' );
                    $host = sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) );
                    $request_uri = esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) );
                    $current_link = urlencode( $protocol . "://" . $host . $request_uri );
                } else {
                    // Handle the case where the SERVER variables are not set
                    $current_link = '';
                }
        ...
        function wpop_get_account() {
            ...
            if ( isset( $_POST['service_provider'] ) ) {
                // Unslash and sanitize the service provider
                $service_name = sanitize_text_field( wp_unslash( $_POST['service_provider'] ) );
            } else {
                // Handle the case where service_provider is not set
                $service_name = '';
            }
            /*
            ...
            if ( $wpOptins->have_posts() ) {
                ...
                while ( $wpOptins->have_posts() ) {
                    /* Making it post. */
                    $wpOptins->the_post();
                    $wpoptin_id = ( isset( $wpoptin_id ) ? $wpoptin_id : '' );
        ...
        function wpop_save_offer_bar() {
            ...
            $wpop_new_title = $parsed_data['wpop_new_title'];
            if ( empty( $wpop_new_title ) ) {
                $wpop_new_title = 'Offer';
            }
            $wpop_edit_id = (int) sanitize_text_field( $parsed_data['wpop_edit_id'] );
            ...
            $wpop_new_post = [
                ...
                'post_content' => htmlentities( sanitize_text_field( $parsed_data['wpop_new_content'] ) ),
                'post_type'    => 'wpoptins',
                'post_status'  => $status,
            ...
            if ( $wpop_edit_id ) {
                ...
                if ( isset( $_POST['wpop_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wpop_nonce'] ) ), 'wpop-ajax-nonce' ) ) {
            ...
            $wpop_new_timer = intval( strtotime( $parsed_data['wpop_new_timer'] ) );
            /* wp_insert_post() insers new post in db. */
            if ( isset( $wpop_new_postID ) ) {
                $wpop_feat_img = ( isset( $parsed_data['wpop_feat_img'] ) ? $parsed_data['wpop_feat_img'] : '' );
                ...
                $wpop_new_goal = ( isset( $parsed_data['wpop_new_goal'] ) ? $parsed_data['wpop_new_goal'] : '' );
                $wpop_new_type = ( isset( $parsed_data['wpop_new_type'] ) ? $parsed_data['wpop_new_type'] : '' );
                $wpop_new_offer_btn_text = ( isset( $parsed_data['wpop_new_offer_btn_text'] ) ? $parsed_data['wpop_new_offer_btn_text'] : '' );
                $wpop_new_offer_btn_url = ( isset( $parsed_data['wpop_new_offer_btn_url'] ) ? $parsed_data['wpop_new_offer_btn_url'] : '' );
                ...
                if ( !$wpop_edit_id ) {
                    ...
                    /* Encoding the url to enhance the awesomeness. */
                    $wpop_url_c = urlencode( $wpop_url_c );
                    $wpop_skin_url = admin_url( 'customize.php?wpop_optin_id=' . $wpop_new_postID . '&url=' . $wpop_url_c . '&autofocus[panel]=wpop_customizer_panel&goal=' . $parsed_data['wpop_new_goal'] . '&type=' . $parsed_data['wpop_new_type'] . '&wpop_skin_id=' . $skin_id . '' );
                    $wpop_skin_html = ( isset( $wpop_skin_html ) && !empty( $wpop_skin_html ) ? $wpop_skin_html : '' );
                    if ( $skin_id ) {
                ...
                $thumbnail_set = ( isset( $thumbnail_set ) ? $thumbnail_set : '' );
        ...
        function wpop_save_phone_calls() {
            ...
            if ( isset( $wpop_new_postID ) ) {
                ...
                update_post_meta( $wpop_new_postID, '_wpop_goal', sanitize_text_field( $parsed_data['wpop_new_goal'] ) );
                update_post_meta( $wpop_new_postID, '_wpop_type', sanitize_text_field( $parsed_data['wpop_new_type'] ) );
                update_post_meta( $wpop_new_postID, '_wpop_country_code', sanitize_text_field( $parsed_data['wpop_country_code'] ) );
                update_post_meta( $wpop_new_postID, 'wpop_new_offer_btn_text', sanitize_text_field( $parsed_data['wpop_new_offer_btn_text'] ) );
                update_post_meta( $wpop_new_postID, 'wpop_new_offer_btn_url', $parsed_data['wpop_new_offer_btn_url'] );
                if ( !$wpop_edit_id ) {
                    $skin_title = $wpop_new_title . ' ' . __( 'Design', 'wpoptin' );
                    $skin_description = __( 'Default design settings for ', 'wpoptin' ) . $wpop_new_title;
                    $skin_data = [
        ...
        function wpop_save_announcement() {
            ...
            if ( isset( $wpop_new_postID ) ) {
                ...
                $wpop_new_goal = ( isset( $parsed_data['wpop_new_goal'] ) ? $parsed_data['wpop_new_goal'] : '' );
                $wpop_new_type = ( isset( $parsed_data['wpop_new_type'] ) ? $parsed_data['wpop_new_type'] : '' );
                $wpop_enable_cupon = ( isset( $parsed_data['wpop_enable_cupon'] ) ? $parsed_data['wpop_enable_cupon'] : '' );
                $wpop_new_cupon = ( isset( $parsed_data['wpop_new_cupon'] ) ? $parsed_data['wpop_new_cupon'] : '' );
                $wpop_new_timer_text = ( isset( $parsed_data['wpop_new_timer_text'] ) ? $parsed_data['wpop_new_timer_text'] : '' );
                $wpop_new_timer = ( isset( $parsed_data['wpop_new_timer'] ) ? $parsed_data['wpop_new_timer'] : '' );
                $wpop_new_offer_btn_text = ( isset( $parsed_data['wpop_new_offer_btn_text'] ) ? $parsed_data['wpop_new_offer_btn_text'] : '' );
                $wpop_new_offer_btn_url = ( isset( $parsed_data['wpop_new_offer_btn_url'] ) ? $parsed_data['wpop_new_offer_btn_url'] : '' );
                update_post_meta( $wpop_new_postID, '_wpop_goal', sanitize_text_field( $wpop_new_goal ) );
                update_post_meta( $wpop_new_postID, '_wpop_type', sanitize_text_field( $wpop_new_type ) );
        ...
        function wpop_save_social_traffic() {
            ...
            if ( isset( $wpop_new_postID ) ) {
                ...
                $wpop_new_goal = ( isset( $parsed_data['wpop_new_goal'] ) ? $parsed_data['wpop_new_goal'] : '' );
                $wpop_new_type = ( isset( $parsed_data['wpop_new_type'] ) ? $parsed_data['wpop_new_type'] : '' );
                $wpop_url_type = ( isset( $parsed_data['wpop_url_type'] ) ? $parsed_data['wpop_url_type'] : '' );
                $wpop_enable_social_current_url = ( isset( $parsed_data['wpop_enable_social_current_url'] ) ? $parsed_data['wpop_enable_social_current_url'] : '' );
                $wpop_new_social_url = ( isset( $parsed_data['wpop_new_social_url'] ) ? $parsed_data['wpop_new_social_url'] : '' );
                update_post_meta( $wpop_new_postID, '_wpop_goal', sanitize_text_field( $wpop_new_goal ) );
                update_post_meta( $wpop_new_postID, '_wpop_type', sanitize_text_field( $wpop_new_type ) );
        ...
        /* wpop_save_woo_product will add goal in database.
         * Returns nothing.
         * Add the value on click.
         */
        function wpop_save_woo_product() {
            /* Getting service provider name. */
            $form_data = $_POST['form_data'];
            $return_url = urlencode( $_POST['return_url'] );
            parse_str( $form_data, $parsed_data );
            $wpop_new_title = $parsed_data['wpop_new_title'];
            if ( empty( $wpop_new_title ) ) {
                $wpop_new_title = __( 'Woocommerce Product', 'wpoptin' );
            }
            $wpop_edit_id = (int) sanitize_text_field( $parsed_data['wpop_edit_id'] );
            if ( isset( $wpop_edit_id ) && !empty( $wpop_edit_id ) ) {
                $status = get_post_status( $wpop_edit_id );
            ...
            $wpop_new_content = ( isset( $parsed_data['wpop_new_content'] ) ? $parsed_data['wpop_new_content'] : '' );
            /* Arguments for new post. */
            $wpop_new_post = [
                'post_title'   => sanitize_text_field( $wpop_new_title ),
            ...
            if ( isset( $wpop_new_postID ) ) {
                ...
                $wpop_new_type = ( isset( $parsed_data['wpop_new_type'] ) ? $parsed_data['wpop_new_type'] : '' );
                $wpop_url_type = ( isset( $parsed_data['wpop_url_type'] ) ? $parsed_data['wpop_url_type'] : '' );
                $wpop_new_woo_product = ( isset( $parsed_data['wpop_new_woo_product'] ) ? $parsed_data['wpop_new_woo_product'] : '' );
                $wpop_enable_social_current_url = ( isset( $parsed_data['wpop_enable_social_current_url'] ) ? $parsed_data['wpop_enable_social_current_url'] : '' );
                $wpop_new_social_url = ( isset( $parsed_data['wpop_new_social_url'] ) ? $parsed_data['wpop_new_social_url'] : '' );
                update_post_meta( $wpop_new_postID, '_wpop_goal', sanitize_text_field( $wpop_new_goal ) );
                update_post_meta( $wpop_new_postID, '_wpop_type', sanitize_text_field( $wpop_new_type ) );
                update_post_meta( $wpop_new_postID, '_wpop_woo_product', sanitize_text_field( $wpop_new_woo_product ) );
                update_post_meta( $wpop_new_postID, '_wpop_url_type', sanitize_text_field( $wpop_url_type ) );
                update_post_meta( $wpop_new_postID, '_wpop_social_url', esc_url_raw( $wpop_new_social_url ) );
                if ( !$wpop_edit_id ) {
                    $skin_title = $wpop_new_title . ' ' . __( 'Design', 'wpoptin' );
        ...
        function wpop_save_custom() {
            ...
            $wpop_new_content = wp_kses( $parsed_data['wpop_new_content'], wp_kses_allowed_html( 'post' ) );
            /* Arguments for new post. */
            $wpop_new_post = [
                'post_title'   => sanitize_text_field( $wpop_new_title ),
            ...
            if ( isset( $wpop_new_postID ) ) {
                ...
                update_post_meta( $wpop_new_postID, '_wpop_goal', sanitize_text_field( $parsed_data['wpop_new_goal'] ) );
                update_post_meta( $wpop_new_postID, '_wpop_type', sanitize_text_field( $parsed_data['wpop_new_type'] ) );
                $wpop_enable_content = ( isset( $parsed_data['wpop_enable_content'] ) ? $parsed_data['wpop_enable_content'] : '' );
                $wpop_enable_button = ( isset( $parsed_data['wpop_enable_button'] ) ? $parsed_data['wpop_enable_button'] : '' );
                $wpop_new_offer_btn_text = ( isset( $parsed_data['wpop_new_offer_btn_text'] ) ? $parsed_data['wpop_new_offer_btn_text'] : '' );
                $wpop_new_offer_btn_url = ( isset( $parsed_data['wpop_new_offer_btn_url'] ) ? $parsed_data['wpop_new_offer_btn_url'] : '' );
        ...
        function wpop_save_triggers_and_conditions() {
            ...
            $form_data = $_POST['form_data'];
            parse_str( $form_data, $parsed_data );
            $wpop_trigger_method = ( isset( $parsed_data['wpop_trigger_method'] ) ? $parsed_data['wpop_trigger_method'] : '' );
            $wpop_auto_method = ( isset( $parsed_data['wpop_auto_method'] ) ? $parsed_data['wpop_auto_method'] : '' );
            $wpop_sec_value = ( isset( $parsed_data['wpop_sec_value'] ) ? $parsed_data['wpop_sec_value'] : '' );
            $wpop_scroll_method = ( isset( $parsed_data['wpop_scroll_method'] ) ? $parsed_data['wpop_scroll_method'] : '' );
            $wpop_scrol_perc_value = ( isset( $parsed_data['wpop_scrol_perc_value'] ) ? $parsed_data['wpop_scrol_perc_value'] : '' );
            $wpop_scrol_slect_value = ( isset( $parsed_data['wpop_scrol_slect_value'] ) ? $parsed_data['wpop_scrol_slect_value'] : '' );
            $wpop_click_class = ( isset( $parsed_data['wpop_click_class'] ) ? $parsed_data['wpop_click_class'] : '' );
            $wpop_pages_method = ( isset( $parsed_data['wpop_pages_method'] ) ? $parsed_data['wpop_pages_method'] : '' );
            $wpop_posts_method = ( isset( $parsed_data['wpop_posts_method'] ) ? $parsed_data['wpop_posts_method'] : '' );
            $wpop_tags_method = ( isset( $parsed_data['wpop_tags_method'] ) ? $parsed_data['wpop_tags_method'] : '' );
            $wpop_cats_method = ( isset( $parsed_data['wpop_cats_method'] ) ? $parsed_data['wpop_cats_method'] : '' );
            $wpop_pages_show_name = ( isset( $parsed_data['wpop_pages_show_name'] ) ? $parsed_data['wpop_pages_show_name'] : '' );
            $wpop_posts_show_name = ( isset( $parsed_data['wpop_posts_show_name'] ) ? $parsed_data['wpop_posts_show_name'] : '' );
            $wpop_tags_show_name = ( isset( $parsed_data['wpop_tags_show_name'] ) ? $parsed_data['wpop_tags_show_name'] : '' );
            $wpop_cats_show_name = ( isset( $parsed_data['wpop_cats_show_name'] ) ? $parsed_data['wpop_cats_show_name'] : '' );
            $wpop_pages_hide_name = ( isset( $parsed_data['wpop_pages_hide_name'] ) ? $parsed_data['wpop_pages_hide_name'] : '' );
            $wpop_posts_hide_name = ( isset( $parsed_data['wpop_posts_hide_name'] ) ? $parsed_data['wpop_posts_hide_name'] : '' );
            $wpop_tags_hide_name = ( isset( $parsed_data['wpop_tags_hide_name'] ) ? $parsed_data['wpop_tags_hide_name'] : '' );
            $wpop_cats_hide_name = ( isset( $parsed_data['wpop_cats_hide_name'] ) ? $parsed_data['wpop_cats_hide_name'] : '' );
            $wpop_show_all = ( isset( $parsed_data['wpop_show_all'] ) ? $parsed_data['wpop_show_all'] : '' );
            $wpop_show_home = ( isset( $parsed_data['wpop_show_home'] ) ? $parsed_data['wpop_show_home'] : '' );
            $wpop_vistor_method = ( isset( $parsed_data['wpop_vistor_method'] ) ? $parsed_data['wpop_vistor_method'] : '' );
            $wpop_vistor_not = ( isset( $parsed_data['wpop_vistor_not'] ) ? $parsed_data['wpop_vistor_not'] : '' );
            $wpop_device_method = ( isset( $parsed_data['wpop_device_method'] ) ? $parsed_data['wpop_device_method'] : '' );
            $wpop_device_not = ( isset( $parsed_data['wpop_device_not'] ) ? $parsed_data['wpop_device_not'] : '' );
            /* Arguments for new post. */
            $wpop_post_cond = [
            ...
            if ( wp_verify_nonce( $_POST['wpop_nonce'], 'wpop-ajax-nonce' ) ) {
                if ( current_user_can( 'editor' ) || current_user_can( 'administrator' ) ) {
                    $id = update_post_meta( $parsed_data['xo_new_id'], '_wpop_conditions', $wpop_post_cond );
                }
            }
            if ( isset( $id ) ) {
                wp_send_json_success( __( 'Saved', 'wpoptin' ) );
                die;
            } else {
                wp_send_json_error( __( 'Something went wrong! Please try again', 'wpoptin' ) );
                die;
        ...
        function wpop_save_optin() {
            ...
            $wpop_new_post = [
                'post_title'   => sanitize_text_field( $wpop_new_title ),
                'post_content' => sanitize_text_field( $parsed_data['wpop_new_content'] ),
                'post_type'    => 'wpoptins',
                'post_status'  => 'draft',
                'post_author'  => get_current_user_id(),
            ];
            $wpop_edit_id = $parsed_data['wpop_edit_id'];
            if ( $wpop_edit_id ) {
                $wpop_new_post['ID'] = $wpop_edit_id;
                $wpop_new_postID = wp_update_post( $wpop_new_post );
            } else {
            ...
            if ( isset( $wpop_new_postID ) ) {
                ...
                $wpop_new_goal = ( isset( $parsed_data['wpop_new_goal'] ) ? $parsed_data['wpop_new_goal'] : '' );
                $wpop_new_type = ( isset( $parsed_data['wpop_new_type'] ) ? $parsed_data['wpop_new_type'] : '' );
                $wpop_new_newsletter_placeholder = ( isset( $parsed_data['wpop_new_newsletter_placeholder'] ) ? $parsed_data['wpop_new_newsletter_placeholder'] : '' );
                $wpop_new_newsletter_btn_text = ( isset( $parsed_data['wpop_new_newsletter_btn_text'] ) ? $parsed_data['wpop_new_newsletter_btn_text'] : '' );
                $wpop_new_newsletter_sucess_msg = ( isset( $parsed_data['wpop_new_newsletter_sucess_msg'] ) ? $parsed_data['wpop_new_newsletter_sucess_msg'] : '' );
                $wpop_new_newsletter_error_msg = ( isset( $parsed_data['wpop_new_newsletter_error_msg'] ) ? $parsed_data['wpop_new_newsletter_error_msg'] : '' );
                update_post_meta( $wpop_new_postID, '_wpop_goal', sanitize_text_field( $wpop_new_goal ) );
                update_post_meta( $wpop_new_postID, '_wpop_type', sanitize_text_field( $wpop_new_type ) );
                ...
                $thumbnail_set = ( isset( $parsed_data['thumbnail_set'] ) ? $parsed_data['thumbnail_set'] : '' );
                wp_send_json_success( [
                    __( 'Saved', 'wpoptin' ),
                    $wpop_new_postID,
                    '',
        ...
        function wpop_save_acounts_data() {
            ...
            $edit_id = (int) sanitize_text_field( $edit_id );
            ...
            if ( isset( 
                ...
                $thumbnail_set = ( isset( $thumbnail_set ) ? $thumbnail_set : '' );
                wp_send_json_success( [$wpop_new_postID, __( 'Saved', 'wpoptin' ), $wpop_skin_html] );
                die;
            } else {
                wp_send_json_error( __( 'Something went wrong! Please try again', 'wpoptin' ) );
                die;
        ...
        function wpop_mad_mimi_authentication() {
            ...
            if ( !is_wp_error( $theme_request ) && $response_code == 200 ) {
                ...
                if ( !empty( $theme_response ) ) {
                    ...
                    wp_send_json_success( [$html, $wpop_new_accountID, __( 'Successfully authenticated!' )] );
                    die;
                }
            } else {
                wp_send_json_error( wp_remote_retrieve_body( $theme_request ) );
                die;
            }
        }
        ...
        function xo_acount_data_page_func() {
            /* Saving ajax value in variable. */
            $mail_acount_name = ( isset( $_POST['acount_name'] ) ? $_POST['acount_name'] : '' );
            $acount_name = (int) sanitize_text_field( $mail_acount_name );
            $mail_api_key = ( isset( $_POST['api_key'] ) ? $_POST['api_key'] : '' );
            $api_key = sanitize_key( $mail_api_key );
            $mail_post_name = ( isset( $_POST['post_name'] ) ? $_POST['post_name'] : '' );
            $post_name = (string) sanitize_text_field( $mail_post_name );
            $mail_acount_id = ( isset( $_POST['acount_id'] ) ? $_POST['acount_id'] : '' );
            $account_id = (int) sanitize_text_field( $mail_acount_id );
            $api = new MCAPI($api_key);
            $retval = $api->get_lists();
            if ( empty( $retval ) ) {
                wp_send_json_error( $api->api_response['1'] );
                die;
            } else {
                if ( $retval->total_items == 0 ) {
                    wp_send_json_error( __( 'No Lists Found. Please Add one in your MailChimp Account', 'wpoptin' ) );
                    die;
                }
                $lists_array = [];
            ...
            // 		print_r($data_holder);
            // exit();
            wp_send_json_success( [$html, $wpop_new_accountID] );
            die;
        }
        ...
        function wpop_delete_entity() {
            ...
            $remove_id = ( isset( $_POST['remove_id'] ) ? $_POST['remove_id'] : '' );
            $child_id = ( isset( $_POST['child_id'] ) ? $_POST['child_id'] : '' );
            if ( $true_c or $true_p ) {
                $data = [
                    'parent_id'    => $remove_id,
                    'child_id'     => $child_id,
        ...
        function wpop_change_status() {
            ...
            if ( $updated ) {
                wp_send_json_success( __( 'Successfully Updated', 'wpoptin' ) );
            } else {
                wp_send_json_error( esc_html__( 'Something went wrong.', 'wpoptin' ) );
            }
            wp_die();
        }
        ...
        function wpop_remove_featured_img() {
            if ( !wp_verify_nonce( $_POST['wpop_nonce'], 'wpop-ajax-nonce' ) ) {
                if ( !current_user_can( 'editor' ) || !current_user_can( 'administrator' ) ) {
                    wp_send_json_error( esc_html__( 'Nonce not verified! Reload the page and try again', 'wpoptin' ) );
                }
            }
            $deleted = delete_post_thumbnail( $_POST['id'] );
            if ( $deleted ) {
                wp_send_json_success( __( 'Successfully Deleted', 'wpoptin' ) );
            } else {
                wp_send_json_error( esc_html__( 'Something went wrong.', 'wpoptin' ) );
            }
            wp_die();
        ...
        function wpop_create_skin() {
            ...
            $attachment_id = 0;
            $dir = wp_upload_dir();
            if ( false !== strpos( $parsed_data['wpop_skin_feat_img'], $dir['baseurl'] . '/' ) ) {
                // Is URL in uploads directory?
                $file = basename( $parsed_data['wpop_skin_feat_img'] );
                $query_args = [
            ...
            // Update skin
            $skin_data = [
                'post_title'   => $parsed_data['wpop_new_skin_title'],
        ...
        function wpop_save_skin_id() {
            ...
            if ( isset( $meta_id ) ) {
                wp_send_json_success( esc_html__( 'Updated', 'wpoptin' ) );
                die;
            } else {
                wp_send_json_error( esc_html__( 'Something went wrong.', 'wpoptin' ) );
                die;
        ...
        function wpop_customize_skin() {
            ...
            if ( isset( $meta_id ) ) {
                ...
                wp_send_json_error( esc_html__( 'Something went wrong! Please try again', 'wpoptin' ) );
                die;
            }
        }
        ...
        function wpop_edit_skin() {
            ...
            if ( $wpoptins_skins[$skin_id] ) {
                ...
                wp_send_json_success( $arraySkin );
                die;
            } else {
                wp_send_json_error( esc_html__( 'Something went wrong.', 'wpoptin' ) );
                die;
            ...
            switch ( $service_name ) {
                case 'mailchimp':
                    ...
                    if ( wp_remote_retrieve_response_code( $wpop_mc_meta_raw ) != 200 ) {
                        wp_send_json_error( esc_html( wp_remote_retrieve_response_message( $wpop_mc_meta_raw ) ) );
                    }
                    ...
                    if ( !isset( $wpop_mc_lists->lists ) ) {
                        wp_send_json_error( esc_html__( 'No Lists Found. Please Add one in your MailChimp Account', 'wpoptin' ) );
                        wp_die();
                    }
                    $lists_array = [];
                ...
                case 'constant_contact':
                    ...
                    if ( !$wpop_cc_lists ) {
                        wp_send_json_error( esc_html__( 'No Lists Found. Please Add one in your Constant Constant Account', 'wpoptin' ) );
                        wp_die();
                    }
                    $lists_array = [];
                    if ( $wpop_cc_lists ) {
                     ...
                     */
                    $wpOptins = new WP_Query($wpOptins);
                    $html = ( isset( $html ) && !empty( $html ) ? $html : '' );
                    /* If any xOptins are in database. */
                    if ( $wpOptins->have_posts() ) {
                        $html .= '<li class="xo_accounts_li xo_accounts_li_' . $service_name . '">
							<label class="label">' . __( 'Select account name', 'wpoptin' ) . '</label>
         ...
         * 
         */
        public function wpop_ai_generate_text_callback() {
            if ( isset( $_POST['conversation'] ) ) {
                $conversation = array_map( function ( $entry ) {
                    return [
                        'role'    => sanitize_text_field( $entry['role'] ),
                        'content' => sanitize_text_field( $entry['content'] ),
                    ];
                }, $_POST['conversation'] );
                $api_key = '38NwTbVToBrAaS82ZemRCf7b30RTJQzIcnSa5EdN';
                $prompt = '';
                ...
                foreach ( $conversation as $entry ) {
                    if ( $entry['role'] === 'user' ) {
                        ...
                        $prompt .= "AI: " . $entry['content'] . "\n";
                    }
                }
                // Increase the max_tokens value for a longer response
                $max_tokens = 400;
                ...
                if ( isset( $data['generations'][0]['text'] ) ) {
                    $ai_response = trim( $data['generations'][0]['text'] );

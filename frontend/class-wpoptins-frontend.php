...
if ( !class_exists( 'WPOptins_Frontend' ) ) {
    class WPOptins_Frontend {
            ...
            if ( is_customize_preview() ) {
                ...
                if ( $wpoptins ) {
                    foreach ( $wpoptins as $xoptin ) {
                        ...
                        /* Geting cupon code. */
                        $settings = $WPOptins->wpop_get_settings(
                            $xoptin_id,
                            'cupon-code',
                            false,
                            false,
                        ...
                        $cupon = ( isset( $settings[0] ) ? $settings[0] : null );
                        /* Html of offer bar */
                        $returner = null;
                        $optin_goal = $WPOptins->wpop_get_settings(
                            $xoptin_id,
        ...
        function wpop_newsletter() {
            global $wpoptins;
            global $WPOptins;
            $wpop_template_nonce = ( isset( $_POST['wpop_template_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['wpop_template_nonce'] ) ) : '' );
            if ( !wp_verify_nonce( $wpop_template_nonce, 'wpop-template-nonce' ) ) {
                wp_send_json_error( esc_html__( 'Nonce not varified', 'wpoptin' ) );
                wp_die();
            }
            /* Getting service provider name. */
            $email = sanitize_email( wp_unslash( $_POST['email'] ?? '' ) );
            parse_str( $email, $parsed_data );
            $is_child = $parsed_data['xo_newsletter_is_child'];
            $account_id = $WPOptins->wpop_get_settings(
                $parsed_data['xo_newsletter_id'],
                'account_id',
                false,
                false,
                $is_child
            ...
            $api_key = ( isset( $account['api_key'] ) ? base64_decode( $account['api_key'] ) : '' );
            $refresh_access_token = ( isset( $account['refresh_access_token'] ) ? base64_decode( $account['refresh_access_token'] ) : '' );
            $token = ( isset( $account['access_token'] ) ? base64_decode( $account['access_token'] ) : '' );
            $username = ( isset( $account['username'] ) ? base64_decode( $account['username'] ) : '' );
            switch ( $parsed_data['xo_newsletter_service'] ) {
                case 'mailchimp':
                    $api = new MCAPI($api_key);
                    $response = $api->add_subscribers( $parsed_data['xo_newsletter_email'], $list_id );
                    if ( $response->status === 'subscribed' ) {
                        ...
                        wp_send_json_success( $sucess_message );
                        wp_die();
                    } else {
                ...
                case 'constant_contact':
                    ...
                    $cc_api_url = esc_url_raw( 'https://api.constantcontact.com/v2/contacts?email=' . $parsed_data['xo_newsletter_email'] . '&api_key=' . $api_key );
                    $error_message = null;
                    ...
                    if ( !is_wp_error( $cc_api_requested ) && $response_code == 200 ) {
                        $subscribe_arr = sprintf( '{"email_addresses":[{"email_address": "%1$s" }], "lists":[{"id": "%2$s"}]}', sanitize_email( $parsed_data['xo_newsletter_email'] ), esc_html( $list_id ) );
                ...
                case 'campaign_monitor':
                    ...
                    $result = $wrap->add( [
                        'EmailAddress'   => sanitize_email( $parsed_data['xo_newsletter_email'] ),
                        'ConsentToTrack' => "Yes",
                    ] );
                ...
                case 'mad_mimi':
                    ...
                    $check_user_url = esc_url_raw( sprintf(
                        ...
                        rawurlencode( sanitize_email( $parsed_data['xo_newsletter_email'] ) ),
                    ...
                    break;
                case 'mailpoet':
                    wp_send_json_error( __( 'Whoops! MailPoet not included in free version', 'wpoptin' ) );
                    wp_die();
                    break;
                default:
                    wp_send_json_error( __( 'Service provider not found', 'wpoptin' ) );
                    wp_die();
                    break;
            }
        }
        ...
        function wpop_view() {
            ...
            $type = ( isset( $_POST['xo_type'] ) ? sanitize_text_field( wp_unslash( $_POST['xo_type'] ) ) : '' );
        ...
        /* xo_sperate_variants Method ends here. */
    }

    ...
    class CS_REST_CurlTransport extends CS_REST_BaseTransport {
        ...
        function make_call($call_options) {
            ...
            if(!$response && $response !== '') {
                $this->_log->log_message('Error making request with curl_error: '.esc_html(curl_errno($ch)),
                    get_class($this), CS_REST_LOG_ERROR);

                require_once dirname(__FILE__).'/exceptions.php';
                throw new CurlException(esc_html(curl_error($ch)), esc_html(curl_errno($ch)));
            }
...
if (!class_exists('CS_REST_SocketWrapper')) {
    class CS_REST_SocketWrapper {
        ...
        function open($domain, $port) {
            ...
            if(!$this->socket) {
                die('Error making request with '.esc_html($errno).': '.esc_html($errstr));
                return false;
